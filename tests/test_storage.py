...
def test_provision_creates_only_the_user_dir_and_statements(client):
    storage.provision_user_storage(7)

    udir = storage.user_dir(7)
    assert udir.is_dir()
    assert storage.statements_dir(7).is_dir()
    ...
    assert sorted(p.name for p in udir.iterdir()) == ["statements"]
